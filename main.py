...
class Board(tk.Frame):
    ...
    def __init__(self, parent, length, width):  # self=Frame, parent=root
        ...
        self.black_images = {}
        self.white_pieces = ["pyimage1", "pyimage3", "pyimage4", "pyimage5", "pyimage6",
                             "pyimage7"]  # for convenience when calling all white pieces
    ...
    def select_piece(self, button):  # called when a square button is pressed, consists of majority of the movement code
        ...
        if (self.piece_color == "white" and self.turns % 2 == 0) or (
                self.piece_color == "black" and self.turns % 2 == 1) or self.buttons_pressed == 1:  # prevents people from moving their pieces when it's not their turn
                ...
                if self.allowed_piece_move() and self.friendly_fire() == False:  # makes sure the move is legal
                    ...
                    if self.in_check() == True and self.castled == False:  # for some reason it says king is in check after a castle so I set up a variable here that would prevent this code from running
                        self.squares[prev_sq2].config(
                            image=prev_sq2_button_piece)  # reverts movement since king is or would be put into check because of move
    ...
    def allowed_piece_move(
            self):  # checks whether the piece can move to square 2 with respect to their movement capabilities
        ...
        if self.sq1_button["image"] == "pyimage2" or self.sq1_button[
            "image"] == "pyimage9":  # for when this function is called for check
            return False
        ...
        if (self.sq1_button["image"] == wb or self.sq1_button["image"] == bb) and self.clear_path(
            ...
            if abs(int(self.sq1[1]) - int(self.sq2[1])) == abs(self.ranks.find(self.sq1[0]) - self.ranks.find(
                    self.sq2[0])):  # makes sure there is equal change between file and rank movement
                return True
        ...
        if self.sq1_button["image"] == wk or self.sq1_button["image"] == bk:  # king movement
            if (abs(int(self.sq1[1]) - int(self.sq2[1])) < 2) and (
            abs(self.ranks.find(self.sq1[0]) - self.ranks.find(self.sq2[0]))) < 2:  # allows 1 square moves
                return True
        ...
        if self.sq1_button["image"] == wp:  # white pawn movement
            if "2" in self.sq1:  # allows for 2 space jump from starting pos
                if (int(self.sq1[1]) + 1 == int(self.sq2[1]) or int(self.sq1[1]) + 2 == int(self.sq2[1])) and self.sq1[
                    0] == self.sq2[0] and self.sq2_button["image"] == "pyimage2":  # allows 2 sq movement
                    ...
                    if in_front["image"] == "pyimage2":  # makes sure that there is no piece blocking path
                        return True
            if int(self.sq1[1]) + 1 == int(self.sq2[1]) and self.sq1[0] == self.sq2[0] and self.sq2_button[
                "image"] == "pyimage2":  # allows 1 sq movement
                return True
            if int(self.sq1[1]) + 1 == int(self.sq2[1]) and (
            abs(self.ranks.find(self.sq1[0]) - self.ranks.find(self.sq2[0]))) == 1 and self.sq2_button[
                "image"] != "pyimage2":  # allows the capturing of diagonal pieces
                return True

        if self.sq1_button["image"] == bp:  # black pawn movement
            if "7" in self.sq1:  # allows for 2 space jump from starting pos
                if (int(self.sq1[1]) == int(self.sq2[1]) + 1 or int(self.sq1[1]) == int(self.sq2[1]) + 2) and self.sq1[
                    0] == self.sq2[0] and self.sq2_button[
                    "image"] == "pyimage2":  # only allows it to move straight 1 or 2 sq
                    return True
            if int(self.sq1[1]) == int(self.sq2[1]) + 1 and self.sq1[0] == self.sq2[0] and self.sq2_button[
                "image"] == "pyimage2":
                return True
            if int(self.sq1[1]) == int(self.sq2[1]) + 1 and abs(
                    self.ranks.find(self.sq1[0]) - self.ranks.find(self.sq2[0])) == 1 and self.sq2_button[
                "image"] != "pyimage2":  # allows the capturing of diagonal pieces if there is an opponent piece there
                return True

        if (self.sq1_button["image"] == wq or self.sq1_button["image"] == bq) and self.clear_path(
        ...
        if self.sq1_button["image"] == wr or self.sq1_button["image"] == br:  # rook movement
            if (int(self.sq1[1]) == int(self.sq2[1]) or self.sq1[0] == self.sq2[0]) and self.clear_path(
                    "rook"):  # only allows movement within same rank or file
                return True
        return False

    def castle(self):  # checks to see if the move entails a castle, and if a castle is allowed
        if not self.wk_moved:  # makes sure king hasn't moved
            ...
            if self.wr2_moved == False and self.sq2 == "g1":
                for x in range(5,
                               7):  # checks to see if squares in between rook and king are empty and are not a possible move for opponent
                    square_button = self.squares[self.ranks[x] + str(1)]
                    if square_button["image"] != "pyimage2":
                        return False
        ...
        if not self.bk_moved:
            if self.br1_moved == False and self.sq2 == "c8":
                for x in range(1,
                               3):  # checks to see if squares in between rook and king are empty and are not a possible move for opponent
    ...
    def set_squares(self):  # fills frame with buttons representing squares
        ...
        for x in range(8):
            for y in range(8):
                ...
                B = tk.Button(self, bg=self.square_color, activebackground="cyan")
    ...
    def set_pieces(self):  # places pieces in starting positions
        dict_rank1_pieces = {"a1": "r.png", "b1": "n.png", "c1": "b.png", "d1": "q.png", "e1": "k.png", "f1": "b.png",
                             "g1": "n.png", "h1": "r.png"}  # assigning positions with their default pieces
